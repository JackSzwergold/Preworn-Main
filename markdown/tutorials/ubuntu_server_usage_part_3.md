...
Now when your server restarts, `iptables-persistent` will load all of the rules in `/etc/iptables/rules.v4` when the machine comes back up so your `iptables` rules will always be in place.
